...
def run(args):
    ...
    questions = dataset["question"][:args.num_samples]
    answers = dataset["answer"][:args.num_samples]
    ...
    for i in tqdm.tqdm(range(len(questions))):
        ...
        chat_completion = openai.chat.completions.create(
            model=model_name,
            messages=[
                {"role": "system", "content": "Answer very briefly by using at most 4 words."},
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_name', default="search_qa", type=str, choices=["search_qa"])
